...
data %>% filter(diagnosis == "ASD", visit == 1) 
mean(mean_age_ADS$age)

data %>% filter(diagnosis == "TD", visit == 1) 
mean(mean_age_TD$age, na.rm = T)
...
model1 <- lmer(CHI_MLU ~ visit + diagnosis + (1|ID) + (1|ados1) + (1|age), data = data, REML = F)
summary(model1)
