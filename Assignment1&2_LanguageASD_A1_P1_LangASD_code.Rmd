...
```{r}
#1. 
MLU_27 <- unique(clean_train %>% filter(CHI_MLU > 2.7))
unique(MLU_27$ID)

#2
MLU_15_1 <- (clean_train %>%
...
Most_6 <- clean_train %>% filter(Visit == 6) %>%
...
Least_1 <- clean_train %>% filter(Visit == 1) %>%
  arrange(tokens_CHI)
unique(Least_1$ID)
...
3. 
The average is relevant to compare each kid against the overall mean. 

The average that we have calculated however, does not enable us to make a comparison between the two groups, and thereby if there is a difference due to diagnosis. Therefore, for this particular purpose an overall mean might not be as descriptive of the data than for instance an average of children with ADS compared against an average of TD children. 

Generally, it is relevant to keep all the data points in order for us to measure if the mean is a good way of representing the data. Moreover, the raw data is important to look at progress or just general development in the word use of the kids individually and for instance taken other variables such as ADOS into account. 
